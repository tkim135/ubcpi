...
class PeerInstructionXBlock(XBlock, MissingDataFetcherMixin, PublishEventMixin):
    ...
    flag_inappropriate_threshold = Integer(
        ...
        help=_("Defines the threshold for removing a student explanation from "
            "the pool after being reported as inappropriate.")
    )
    ...
    def _submission_considered_inappropriate(self, student_id):
        """
        Checked whether student's submission is considered as inappropriate:
        - marked as inappropriate by instructional team
        - down vote by student is over the threshold
        """
        flagged_item = self.flagged_answers.get(student_id, {})
    ...
    def flag_inappropriate(self, data, suffix=''):
        ...
        if the_id != SEED_EXPLANATION_ID:
            ...
            flagged.setdefault('reported_by', {}).setdefault(str(student_item['student_id']), {})
    ...
    def _pool_status(self):
        ...
        for option in self.sys_selected_answers:
            for student_id in self.sys_selected_answers[option]:
                ...
                if option == str(ans.get_vote(0)):
                    result.append(dict(
                        option=option,
                        id=student_id,
                        explanation=ans.get_rationale(0),
                        inappropriate_report_count=len(self.flagged_answers.get(str(student_id), {}).get('reported_by', {})),
                        considered_inappropriate=self._submission_considered_inappropriate(str(student_id)),
                        staff_set_inappropriate=self.flagged_answers.get(str(student_id), {}).get('staff_set_inappropriate', None),
                    ))
        return result
